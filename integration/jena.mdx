---
title: "Apache Jena Integration: Querying FalkorDB with SPARQL"
sidebarTitle: "Apache Jena"
...
<AccordionGroup>
  <Accordion title="What is the jena-falkordb-adapter used for?">
  ...
  <Accordion title="How do I install the Jena FalkorDB adapter?">
    Add the Maven dependency with groupId `com.falkordb` and artifactId `jena-falkordb-adapter` to your `pom.xml`. Check the [adapter repository](https://github.com/FalkorDB/jena-falkordb-adapter) for the latest version.
  </Accordion>
  <Accordion title="Can I run SPARQL queries against FalkorDB using Jena?">
    Yes, once the adapter is configured, you can run standard **SPARQL SELECT, CONSTRUCT, and ASK** queries using Jena's `QueryExecutionFactory` against data stored in FalkorDB.
  </Accordion>
