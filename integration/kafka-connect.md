...
| --- | --- | --- | --- |
| `graphName` | Specifies the name of the graph database where the command will be executed. | `"falkordb"`. Kafka messages can update multiple graphs. | Identifies target graph. |
| `command` | Indicates the type of operation being performed. `"GRAPH_QUERY"` means a query will be executed against the graph database. | `"GRAPH_QUERY"` | Chooses command type. |
| `cypherCommand` | Contains the actual Cypher query to be executed. Cypher is a query language for graph databases. | `CREATE (p:Person {name: $name_param, age: $age_param, location: $location_param}) RETURN p` | Creates a `Person` node with `name`, `age`, and `location` properties. |
| `parameters` | Holds key-value pairs for placeholders in the `cypherCommand`. | `{"name_param": "Person 0", "age_param": 20, "location_param": "Location 0"}` | Binds values for placeholders. |
