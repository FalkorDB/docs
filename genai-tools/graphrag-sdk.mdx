---
title: "FalkorDB GraphRAG SDK: Fast Knowledge Graph Generation for GenAI"
sidebarTitle: "GraphRAG-SDK"
...
<Note>
  GraphRAG-SDK has its own documentation at [docs.falkordb.com/graphrag](/graphrag). This page is a short overview — the full guides, API reference, and runnable examples live there.
</Note>
...
| [Architecture](/graphrag/architecture) | How the ingestion and retrieval pipeline works |
| [Configuration](/graphrag/configuration) | Connection, providers, and tuning reference |
| [Graph Schema](/graphrag/graph-schema) | Constrain the entity and relation types the extractor produces |
| [Ingestion](/graphrag/ingestion) | Document loading, chunking, extraction, and resolution |
| [Retrieval](/graphrag/retrieval) | Hybrid retrieval, expansion, and reranking |
| [Strategies](/graphrag/strategies) | Swappable strategy interfaces and built-in implementations |
| [Providers](/graphrag/providers) | LLM and embedder configuration |
| [Storage](/graphrag/storage) | How the graph is laid out in FalkorDB |
| [Extraction](/graphrag/extraction) | Entity and relation extraction internals |
| [Incremental Updates](/graphrag/incremental-updates) | `update()`, `delete_document()`, and `apply_changes()` for CI-driven syncs |
| [Ontology Discovery](/graphrag/ontology-discovery) | Auto-draft a schema from a corpus |
| [Ontology Evolution](/graphrag/ontology-evolution) | Safely change a schema on a populated graph |
| [Benchmark](/graphrag/benchmark) | Methodology, results, and reproduction instructions |
...
[![GraphRAG Server: turn your documents into a knowledge graph you can question](/images/graphrag-server-hero.webp)](https://graphrag.falkordb.com/)
...
**Resources:**
- [GraphRAG-SDK documentation](/graphrag)
...
<AccordionGroup>
  <Accordion title="What is GraphRAG-SDK?">
  ...
  <Accordion title="Which LLMs and embedding models does GraphRAG-SDK support?">
    Any LLM or embedder reachable through [LiteLLM](/graphrag/providers), which covers OpenAI, Anthropic, Google, and locally hosted models. You select them with `LiteLLM(model=...)` and `LiteLLMEmbedder(model=...)`.
  </Accordion>
  <Accordion title="What file types can GraphRAG-SDK ingest?">
  ...
  <Accordion title="Do I have to rebuild the whole graph when a document changes?">
    No. GraphRAG-SDK re-syncs individual documents incrementally. See [Incremental Updates](/graphrag/incremental-updates) for `update()`, `delete_document()`, and `apply_changes()`, which are designed for CI-driven syncs.
  </Accordion>
  <Accordion title="Why use GraphRAG instead of regular RAG?">
  ...
  <Accordion title="How accurate is GraphRAG-SDK?">
    It holds the top score on [GraphRAG-Bench](https://graphrag-bench.github.io) across both the multi-document and single-document question sets. The [Benchmark](/graphrag/benchmark) page documents the methodology and how to reproduce the results.
  </Accordion>
  <Accordion title="Where is the full GraphRAG-SDK documentation?">
    This page is a short overview. The complete guides, API reference, and runnable examples live at [docs.falkordb.com/graphrag](/graphrag).
  </Accordion>
</AccordionGroup>
