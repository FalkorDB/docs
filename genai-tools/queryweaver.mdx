---
title: "QueryWeaver: Graph-Powered Natural Language to SQL (Text2SQL)"
sidebarTitle: "QueryWeaver"
...
- [FalkorDB MCP Server](/genai-tools/mcpserver): Enable AI assistants to interact with FalkorDB using MCP
- [GraphRAG-SDK](/genai-tools/graphrag-sdk): Build intelligent GraphRAG applications with FalkorDB and LLMs

## Frequently Asked Questions

<AccordionGroup>
  <Accordion title="What is QueryWeaver?">
  ...
  <Accordion title="Which databases does QueryWeaver support?">
  ...
  <Accordion title="Which AI providers does QueryWeaver support?">
  ...
  <Accordion title="Can I use QueryWeaver with MCP-compatible AI assistants?">
    Yes. QueryWeaver includes built-in MCP server support, exposing tools like `list_databases`, `connect_database`, `database_schema`, and `query_database`. Configure your MCP client (Claude Desktop, VS Code Copilot) to connect to `http://127.0.0.1:5000/mcp`.
  </Accordion>
  <Accordion title="How do I get started quickly?">
    Run `docker run -p 5000:5000 -it falkordb/queryweaver` and open http://localhost:5000 in your browser. Set an LLM provider API key (e.g., `OPENAI_API_KEY`) as an environment variable for AI-powered SQL generation.
  </Accordion>
  <Accordion title="Does QueryWeaver have a REST API?">
    Yes. QueryWeaver exposes REST endpoints for managing graphs and running Text2SQL queries. Key endpoints include `GET /graphs` to list databases, `POST /graphs/{graph_id}` to run queries, and all require Bearer token authentication.
  </Accordion>
</AccordionGroup>
