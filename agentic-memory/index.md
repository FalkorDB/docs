...
{% include faq_accordion.html title="Frequently Asked Questions" q1="What is agentic memory and how does it differ from traditional database storage?" a1="Agentic memory enables AI agents to maintain **persistent, contextual memory** across interactions. Unlike traditional storage, it uses graph structures to represent relationships between entities, supports temporal reasoning, and allows agents to build understanding over time rather than just storing flat records." q2="Why is a graph database like FalkorDB better suited for agentic memory than a relational database?" a2="Graph databases naturally represent entities and their relationships, enabling fast traversals for context-aware retrieval. FalkorDB provides **native multi-graph support** for tenant isolation, hybrid search combining vectors with graph relationships, and temporal tracking - all essential for agentic memory systems." q3="Can I use multiple agentic memory frameworks together with FalkorDB?" a3="Yes, all frameworks (Graphiti, Cognee, Mem0) integrate seamlessly with FalkorDB and can be used together in complex systems. Each framework can connect to the same FalkorDB instance using different graph names for isolation." q4="Which agentic memory framework should I choose?" a4="Choose **Graphiti** for temporal reasoning and multi-agent memory, **Graphiti MCP Server** for adding memory to Claude Desktop or Cursor IDE, **Cognee** for flexible hybrid storage structures, or **Mem0** if you already use Mem0 agents and want graph-structured memory." q5="Do I need an LLM provider API key to use agentic memory with FalkorDB?" a5="Yes, most agentic memory frameworks require an LLM provider (like OpenAI) for entity extraction and embedding generation. The LLM processes unstructured text into structured graph entities and relationships." %}
