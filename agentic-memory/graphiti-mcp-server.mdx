---
title: "Graphiti MCP Server: Model Context Protocol for Agent Memory"
sidebarTitle: "Graphiti MCP Server"
...
- Explore [Graphiti Python Library](/agentic-memory/graphiti) for direct integration
- Learn about [Cognee](/agentic-memory/cognee) for flexible memory management
...
<AccordionGroup>
  <Accordion title="What is the Model Context Protocol (MCP) and why use it with Graphiti?">
  ...
  <Accordion title="Which AI clients are compatible with the Graphiti MCP Server?">
    Any MCP-compatible client works, including **Claude Desktop**, **Cursor IDE**, **VS Code with GitHub Copilot**, and other applications that support the Model Context Protocol. The server uses HTTP transport at `http://localhost:8000/mcp/`.
  </Accordion>
  <Accordion title="What is the difference between the combined and separate container Docker setups?">
  ...
  <Accordion title="How do I fix 429 rate limit errors from the LLM provider?">
    Reduce the `SEMAPHORE_LIMIT` environment variable. For OpenAI Tier 1 (free), use 1-2. For Tier 2, use 5-8. For Tier 3, use 10-15. Monitor your LLM provider dashboard for actual request rates.
  </Accordion>
  <Accordion title="Is the knowledge graph data persisted between Docker restarts?">
    By default, Docker containers lose data on restart. For persistence, use `docker-compose` with properly configured **volumes** for FalkorDB data storage, or use FalkorDB Cloud for managed persistent hosting.
  </Accordion>
</AccordionGroup>
