...
- **Native multi-graph support**: Isolate a tenant in its own graph instead of filtering a shared table — Option B uses this to give every agent's dataset its own graph, subject to the [Per-Agent Graphs](#per-agent-graphs) caveats. Option A keeps one graph and separates users with a `user_id` property instead
...
- **Easy cleanup**: Where memory maps to a graph per tenant, as Option B does per dataset, forgetting one is a graph drop rather than a cascade of deletes. Option A deletes by `user_id` within its single graph, and Option B can also route deletion through Cognee's own API so it clears derived knowledge alongside the raw data
...
- **Team assistants**: Run an assistant per teammate, each gateway configured with its own `userId`, so contexts stay apart — one shared gateway gives everyone the same memory
...
| **Isolation model** | Per user, `user_id` property in one `mem0` graph | Per agent, dataset `<prefix>-<agentId>` in its own graph |
...
| **FalkorDB** | Stores entity relationships, tagged with a `user_id` property |
| **OpenAI** | Embeddings for semantic search and an LLM for entity extraction |

On every turn the plugin **recalls** before the agent answers, injecting relevant memories, and **captures** afterwards, extracting new entities and relationships into the graph under the session's effective `user_id`. For the main agent that is the `userId` you configure; a named agent gets `<userId>:agent:<agentId>` instead, and sub-agent sessions are skipped entirely so the main agent captures the consolidated result.
...
Graph names come from `graphStore.config.graphName` alone, so the `mem0` above is the whole name. `@falkordb/mem0` selects that one graph at startup and separates users with a `user_id` property on every node, rather than giving each user a graph. The `userId` you set is the value written to that property.

<Warning>
`userId` is static gateway configuration, not a per-speaker identity. On a shared gateway everyone talking to the assistant writes and reads the same `user_id`. To keep people apart you need a gateway per person, or an agent that passes an explicit `userId` to the `memory_search` and `memory_store` tools on every call.

The one value the plugin does derive is per agent, not per person: a named agent captures under `<userId>:agent:<agentId>`, while the main agent uses `userId` unchanged and sub-agent sessions are skipped. So `alice` covers the main agent, and a `research` agent writes `alice:agent:research`.
</Warning>
...
<Note>
This applies to open-source mode, where the graph lives in your FalkorDB instance. In [Platform Mode](#platform-mode) there is no `mem0` graph to open, because memory is stored in Mem0 Cloud — browse it from the [Mem0 dashboard](https://app.mem0.ai) instead.
</Note>

For a local container, open the [FalkorDB Browser](/browser) at [http://localhost:3000](http://localhost:3000); for FalkorDB Cloud, use the Browser in [app.falkordb.cloud](https://app.falkordb.cloud). Select the graph `mem0`, substituting your own `graphName` if you changed it, and run:

```cypher
MATCH (n) RETURN DISTINCT n.user_id
```

That lists the IDs actually in use, which is worth doing first: a named agent writes `alice:agent:research` rather than `alice`. Then filter on one of them:

```cypher
MATCH (a)-[r]->(b) WHERE a.user_id = 'alice' RETURN a, r, b
```

You should see extracted entities such as people, technologies, and hobbies, joined by the relationships between them. Drop the `WHERE` clause to see every ID's memories at once. The same check from the terminal — `GRAPH.LIST` prints the real graph name:
...
GRAPH.LIST
GRAPH.QUERY mem0 "MATCH (n) RETURN DISTINCT n.user_id"
GRAPH.QUERY mem0 "MATCH (n) WHERE n.user_id = 'alice' RETURN n.name LIMIT 10"
GRAPH.QUERY mem0 "MATCH (a)-[r]->(b) WHERE a.user_id = 'alice' RETURN a.name, type(r), b.name"
```
...
<Note>
This applies to the local Compose stack only. It starts its own FalkorDB service, named `falkordb` on port `6379`, which clashes with a standalone container from [Running FalkorDB](#running-falkordb) on both the container name and the port. Remove that container first with `docker rm -f falkordb`, which works whether it is running or already stopped. If you came straight here and never created it, the command prints `Error response from daemon: No such container: falkordb` and still exits successfully, so you can ignore that line and carry on. Its data is safe: the `falkordb_data` volume outlives the container. Compose does create its own project-scoped volume rather than reusing that one, so the stack starts from an empty graph while the earlier memories stay in the standalone volume. If you followed [Using FalkorDB Cloud Instead](#using-falkordb-cloud-instead), there is no local database service and nothing to remove.
</Note>
...
<Warning>
Do not continue unless **all four** checks pass: `graph falkor: True`, `vector falkor: True`, both handlers present, and `PONG`. Graph support can register while vector support does not, and that combination breaks hybrid memory later with no error at setup time, so treating `graph falkor: True` alone as success is what makes this failure hard to diagnose.

The two kinds of failure have different fixes. A registration failure — a `False`, or a missing handler — is a build problem: confirm `/app/sitecustomize.py` exists in the container and that the adapter install succeeded in the build logs, then rebuild. A failed `PING` is a connectivity problem, and rebuilding will not change it: on the local stack check that the `falkordb` service is up, and on Cloud check the host, port, username, password, and whether the instance requires TLS, which this adapter cannot do.
</Warning>
...
- **Graph stays empty**: on the self-hosted paths, list graphs with `GRAPH.LIST` to confirm the expected name. Under Option A there is one graph, named by `graphName`, so an empty result means nothing has been captured yet — a changed `userId` does not create a new graph, it only changes the `user_id` written on new nodes, which can make existing memories look missing. Option B names each graph after its dataset's UUID rather than after the agent, so match a graph to an agent through the Cognee `/api/v1/datasets` listing rather than by reading the name. An empty graph there usually means the session cache has not been bridged into Cognee yet, which only happens on `session_end` or when you run `openclaw cognee improve --dataset <name>`, so expect it to stay empty until one of those runs. In [Platform Mode](#platform-mode) there is no local graph to list — check the [Mem0 dashboard](https://app.mem0.ai) instead.
...
- **Memories seem to be missing**: they are filtered by the `user_id` property, not stored in separate graphs, so a changed `userId` hides earlier memories rather than deleting them. Run `MATCH (n) RETURN DISTINCT n.user_id` against `mem0` to see which IDs exist — a named agent writes `<userId>:agent:<agentId>`, so filtering on the bare `userId` misses its memories.
- **Two people share one memory**: expected on a single gateway. `userId` is static configuration, and the only ID the plugin derives is per named agent, never per speaker. Run a gateway per person, or have the agent pass an explicit `userId` to the memory tools.
...
<AccordionGroup>
  <Accordion title="Should I use the Mem0 plugin or the Cognee integration with OpenClaw?">
    Use the **Mem0 plugin** for the fastest path: install `@falkordb/openclaw-mem0`, edit one config block, and you have persistent graph memory. Use the **Cognee integration** when you want FalkorDB to serve both vectors and the graph, or need Cognee's processing pipeline. Neither gives you a per-speaker boundary out of the box: Mem0 tags nodes with a static configured `user_id`, so separating people means a gateway each, while Cognee routes per agent but can still fall back to a shared agent-scope index on the release documented here, letting one agent read a dataset belonging to a different agent — see [Per-Agent Graphs](#per-agent-graphs).
  </Accordion>
  <Accordion title="How does OpenClaw memory stay isolated between users or agents?">
    The two paths draw the boundary in different places. The Mem0 plugin keeps every memory in one graph, named by `graphStore.config.graphName` and defaulting to `mem0`, and tags each node with a `user_id` property that queries filter on. That `userId` is static gateway configuration, so a shared gateway means a shared memory space and separating people means running a gateway each. The Cognee integration separates per agent instead: each agent gets its own Cognee dataset, and because the recipe here runs with backend access control and the per-dataset handler, each dataset gets its own FalkorDB graph named after the dataset's UUID. See [Per-Agent Graphs](#per-agent-graphs) for the caveats before relying on it. Option A's platform mode is different again: memory lives in Mem0 Cloud, so isolation is whatever Mem0 provides for your account rather than a FalkorDB graph boundary.
  </Accordion>
