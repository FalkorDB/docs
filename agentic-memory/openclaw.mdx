...
| **Isolation model** | Per user, graph `mem0_alice` | Shared by default; opt-in per-agent graph per Cognee dataset |
...
Per-agent isolation is **strictly opt-in and never enabled automatically**, no matter how many agents the gateway hosts. By default every agent reads and writes the same dataset, `agent_sessions`, and therefore the same FalkorDB graph. Agents stay distinguishable inside it, because each session registers under its own Cognee session ID, but they do share knowledge.

<Warning>
Isolation requires **both** `perAgentMemory: true` and one of `agentDatasetPrefix` or `agentDatasetTemplate`. Setting `perAgentMemory` on its own does nothing, and setting only a prefix leaves the default shared dataset in place. If you expect one graph per agent, verify it rather than assuming: with only one of the two set, every agent keeps writing to the shared graph and one agent can recall memories written by another.
</Warning>

With both set, as in the configuration above, each agent's files and chat route to a dataset named `agentDatasetPrefix` followed by a lowercased agent ID, and each dataset gets its own FalkorDB graph. Use `agentDatasetTemplate`, for example `"{agentId}"`, instead of the prefix when you need full control over dataset names. Give every agent its own `workspace` in `agents.list`, and seed a one-line `MEMORY.md` in each so its dataset is created when the gateway starts.

Shared memory is often what you want, since agents benefit from each other's knowledge. Choose it deliberately rather than by omission.
...
For a quick isolation check, first confirm that both `perAgentMemory` and a dataset prefix are set, then tell agent A something, end its session, and ask agent B. Agent B should not know it. If it does, isolation is not active and the agents are sharing the default dataset.
...
- **Agents can see each other's memories**: per-agent isolation is opt-in. Confirm that both `perAgentMemory: true` and `agentDatasetPrefix` (or `agentDatasetTemplate`) are set. With only one of them, every agent shares the default `agent_sessions` dataset and its graph, and nothing warns you.
...
<AccordionGroup>
  ...
  <Accordion title="How does OpenClaw memory stay isolated between users or agents?">
    In the self-hosted configurations, both paths rely on FalkorDB's **native multi-graph support**. The Mem0 plugin writes each user to their own graph, named `graphName` followed by the user ID, such as `mem0_alice`. The Cognee integration can give each agent its own dataset, and each dataset maps to its own FalkorDB graph, but that isolation is opt-in: it requires both `perAgentMemory: true` and an `agentDatasetPrefix` or `agentDatasetTemplate`, otherwise every agent shares one dataset and one graph. Where isolation is active, no user ID filtering is needed in queries, and forgetting someone is a single graph drop. Option A's platform mode is different again: memory lives in Mem0 Cloud, so isolation is whatever Mem0 provides for your account rather than a FalkorDB graph boundary.
  </Accordion>
