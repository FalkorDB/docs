...
On every turn the plugin **recalls** before the agent answers, injecting relevant memories, and **captures** afterwards, extracting new entities and relationships into the graph under the configured `user_id`.
...
<Warning>
`userId` is static gateway configuration, not a per-speaker identity. Auto-capture derives `<userId>:agent:<agentId>` for agent and sub-agent sessions, but never per end user, so on a shared gateway everyone talking to the assistant writes and reads the same `user_id`. To keep people apart you need a gateway per person, or an agent that passes an explicit `userId` to the `memory_search` and `memory_add` tools on every call.
</Warning>
...
You should see extracted entities such as people, technologies, and hobbies, joined by the relationships between them. Drop the `WHERE` clause to see every user's memories at once. The same check from the terminal — `GRAPH.LIST` prints the real graph name:
...
- **Memories seem to be missing**: they are filtered by the `user_id` property, not stored in separate graphs, so a changed `userId` hides earlier memories rather than deleting them. Query `mem0` without a `WHERE` clause to see everything, and confirm `graphName` and `userId` in your config.
- **Two people share one memory**: expected on a single gateway. `userId` is static configuration, and auto-capture only derives a distinct ID for agent and sub-agent sessions, never per speaker. Run a gateway per person, or have the agent pass an explicit `userId` to the memory tools.
