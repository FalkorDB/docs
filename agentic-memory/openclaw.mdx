...
- **Native multi-graph support**: Isolate a tenant in its own graph instead of filtering a shared table — Option A uses this to give every user their own graph, and Option B to give every agent's dataset one
