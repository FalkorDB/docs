...
[OpenClaw](https://github.com/openclaw/openclaw) is a personal AI assistant runtime that talks to you over channels like WhatsApp, Telegram, Slack, and the CLI. By default its memory is short-lived: end the session and everything you said is gone, so the next conversation starts from zero.

FalkorDB replaces that scratchpad with a persistent knowledge graph. Facts, preferences, and the relationships between them are stored as nodes and edges, so your assistant can recall not just *what* you said but *how it all connects*. For background on why a graph beats flat files and plain vector search for agent memory, see [Curing AI Amnesia](https://www.falkordb.com/blog/curing-ai-amnesia/).
...
- [Node.js 22 or later](https://nodejs.org/), required by OpenClaw
...
Port `6379` serves the database and port `3000` serves the built-in [FalkorDB Browser](/browser). Verify it is up:
...
Sign up at [app.falkordb.cloud](https://app.falkordb.cloud), create an instance, and note its **host**, **port**, **username**, and **password**. Use those values wherever the examples below show `localhost`.
...
Open the [FalkorDB Browser](/browser) at [http://localhost:3000](http://localhost:3000), select the graph `mem0_alice`, and run:
...
Both the `falkordb` and `cognee` containers should be present.
...
Expect `graph falkor: True`, `vector falkor: True`, handlers `falkor_graph_local` and `falkor_vector_local`, and `PONG`.

If `graph falkor` is `False`, the adapter did not register. Confirm `/app/sitecustomize.py` exists in the container and that the adapter install succeeded in the build logs, then rebuild. Do not continue until this prints `True`.

### Point OpenClaw at Cognee

The `cognee-openclaw` plugin only needs the API URL. In `~/.openclaw/openclaw.json`:
...
- **Cannot reach FalkorDB**: run `redis-cli -h localhost -p 6379 ping` and confirm it returns `PONG`. Check the host, port, and credentials in your configuration.
