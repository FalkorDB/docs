...
- **Graph stays empty**: on the self-hosted paths, list graphs with `GRAPH.LIST` to confirm the expected name. Under Option A the graph is keyed by `userId`, so a changed identifier means a new, empty graph. Option B is different: every agent shares the `agent_sessions` dataset and its graph, so changing an agent identifier does **not** create a new one — an empty graph there usually means the session cache was never bridged, which happens on `session_end` or when you run `openclaw cognee improve --dataset <name>`. In [Platform Mode](#platform-mode) there is no local graph to list — check the [Mem0 dashboard](https://app.mem0.ai) instead.
