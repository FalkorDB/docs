...
- **Multi-agent gateways**: Host several agents on one gateway, each with its own slice of memory. Option B gives every agent a dataset of its own inside a shared graph — see [Per-Agent Graphs](#per-agent-graphs) for what that does and does not guarantee
...
| **Isolation model** | Per user, graph `mem0_<userId>` | Per agent, dataset `<prefix>-<agentId>`, inside one shared graph |
...
Separation is enforced by Cognee, not by a graph boundary. The FalkorDB adapter points every dataset at a single graph named `cognee_graph` unless you set `GRAPH_DATABASE_NAME`, so `GRAPH.LIST` shows one graph no matter how many agents you run. Every agent's nodes live in that graph together, and recall stays scoped because Cognee filters by dataset. The adapter does ship a per-dataset handler that names a graph after each dataset ID, but it is opt-in and the stack above does not enable it.
...
For a quick separation check, tell agent A something, end its session, then ask agent B. Agent B should not know it, because the two write different datasets. If it does, confirm both agents resolved the dataset name you expect rather than falling back to the shared agent-scope index described above.
...
Add `--tls` to the Cloud commands if the instance has TLS enabled. The graph is `cognee_graph` unless you set `GRAPH_DATABASE_NAME`, so expect one graph rather than one per agent.
...
- **Graph stays empty**: on the self-hosted paths, list graphs with `GRAPH.LIST` to confirm the expected name. Under Option A the graph is keyed by `userId`, so a changed identifier means a new, empty graph. Option B is different: the graph name is fixed — `cognee_graph` unless you set `GRAPH_DATABASE_NAME` — and agents are separated by dataset inside it, so changing an agent identifier gives you a new dataset but never a new graph. An empty graph there usually means the session cache has not been bridged into Cognee yet, which only happens on `session_end` or when you run `openclaw cognee improve --dataset <name>`, so expect it to stay empty until one of those runs. In [Platform Mode](#platform-mode) there is no local graph to list — check the [Mem0 dashboard](https://app.mem0.ai) instead.
...
<AccordionGroup>
  <Accordion title="Should I use the Mem0 plugin or the Cognee integration with OpenClaw?">
    Use the **Mem0 plugin** for the fastest path: install `@falkordb/openclaw-mem0`, edit one config block, and you have per-user graph memory. Use the **Cognee integration** when you want FalkorDB to serve both vectors and the graph, or need Cognee's processing pipeline. If you need memory kept separate per user, pick Mem0, which gives each user their own graph. The Cognee plugin separates per agent instead, by dataset rather than by graph — see [Per-Agent Graphs](#per-agent-graphs).
  </Accordion>
  <Accordion title="How does OpenClaw memory stay isolated between users or agents?">
    The two paths draw the boundary in different places. The Mem0 plugin uses FalkorDB's **native multi-graph support**, writing each user to their own graph named `graphName` followed by the user ID, such as `mem0_alice` — so per-user separation is a real graph boundary, no ID filtering is needed in queries, and forgetting someone is a single graph drop. The Cognee integration separates per agent instead, giving each agent its own Cognee dataset while keeping every dataset in one shared graph, so the boundary is enforced by Cognee's dataset filtering rather than by FalkorDB. See [Per-Agent Graphs](#per-agent-graphs) for the caveats before relying on it. Option A's platform mode is different again: memory lives in Mem0 Cloud, so isolation is whatever Mem0 provides for your account rather than a FalkorDB graph boundary.
  </Accordion>
