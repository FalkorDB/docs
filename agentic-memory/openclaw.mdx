...
| **Isolation model** | Per user, graph `mem0_alice` | Shared across agents on the current release |
...
`docker compose down` stops the stack but keeps the data; adding `-v` also deletes the stack's volumes. On the local stack that wipes both the Cognee and FalkorDB volumes. On the [Cloud](#using-falkordb-cloud-instead) variant there is no `falkordb_data` volume, so `-v` removes only the Cognee volume and leaves the remote graph untouched — clear that through the API below or from the Cloud console. To clear memory without destroying the containers, wipe through the API and then reset the plugin's local state:
