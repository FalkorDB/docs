...
What those files cannot capture is **relationships**. Flat text has no edges, so the agent re-reads whole files each turn, which is slow and token-hungry, and any question that needs a join stays out of reach. Plain vector search has the same blind spot: it returns similar snippets, never the connection between them.
...
```bash
docker run -d --name falkordb -p 127.0.0.1:6379:6379 -p 127.0.0.1:3000:3000 falkordb/falkordb:latest
```
...
Remove the `depends_on` block along with the `falkordb` service, since there is no local database to wait for, and set `FALKORDB_PASSWORD` in your `.env` file next to `LLM_API_KEY`. Both the graph and vector settings must point at the same instance, because this adapter uses FalkorDB for both.
