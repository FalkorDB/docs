...
- [Node.js](https://nodejs.org/) 24.16 or later, or 26.1 or later, as required by OpenClaw's `engines` range. Node 26 is recommended and Node 22 is not supported
...
<Note>
This Compose file starts its own FalkorDB service, named `falkordb` on port `6379`. If you already started a standalone container in [Running FalkorDB](#running-falkordb), stop and remove it first with `docker rm -f falkordb`, otherwise the build fails on a container name and port clash.
</Note>
