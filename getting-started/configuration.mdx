---
title: "Configuration"
description: "Configure FalkorDB with Redis settings and module parameters for threads, cache, timeouts, query limits, and persistence. Adjust settings at load-time or runtime."
---
...
Before FalkorDB v4.20.2, `EFFECTS_THRESHOLD` is 300 μs. Starting with
FalkorDB v4.20.2, it is 0 μs.
...
<AccordionGroup>
  <Accordion title="What is the difference between load-time and run-time configuration?">
    **Load-time** parameters can only be set when the FalkorDB module is loaded (via Docker env vars, redis.conf, or MODULE LOAD command). **Run-time** parameters can also be changed while the server is running using `GRAPH.CONFIG SET`. Note that run-time changes are not persisted after server restart.
  </Accordion>
  <Accordion title="How do I set configuration parameters with Docker?">
    Use the `FALKORDB_ARGS` environment variable: `docker run -p 6379:6379 -e FALKORDB_ARGS='THREAD_COUNT 4 CACHE_SIZE 50' --rm falkordb/falkordb:latest`. For Redis settings like authentication, use the `REDIS_ARGS` variable: `-e REDIS_ARGS='--requirepass mypassword'`.
  </Accordion>
  <Accordion title="How many threads should I configure for FalkorDB?">
    The `THREAD_COUNT` parameter defaults to your system's logical core count. This controls the maximum concurrent queries. For production workloads, tune this based on your query patterns and available CPU cores. Note that `OMP_THREAD_COUNT` separately controls parallelism *within* individual queries for GraphBLAS computations.
  </Accordion>
  <Accordion title="How do I limit query memory usage and execution time?">
    Use `QUERY_MEM_CAPACITY` to set a memory limit (in bytes) per query, and `TIMEOUT_DEFAULT`/`TIMEOUT_MAX` to set execution time limits (in milliseconds). Both can be changed at runtime with `GRAPH.CONFIG SET`. A value of 0 means unlimited. Note: the older `TIMEOUT` parameter is deprecated since v2.10.
  </Accordion>
  <Accordion title="How can I check the current configuration values?">
    Use `GRAPH.CONFIG GET *` to retrieve all current configuration values, or `GRAPH.CONFIG GET PARAMETER_NAME` for a specific parameter. These commands work from any Redis client or redis-cli.
  </Accordion>
</AccordionGroup>
