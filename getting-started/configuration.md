...
```sh
127.0.0.1:6379> MODULE LOAD falkordb.so [OPT VAL]...
```
...
{% include faq_accordion.html title="Frequently Asked Questions" q1="What is the difference between load-time and run-time configuration?" a1="**Load-time** parameters can only be set when the FalkorDB module is loaded (via Docker env vars, redis.conf, or MODULE LOAD command). **Run-time** parameters can also be changed while the server is running using `GRAPH.CONFIG SET`. Note that run-time changes are not persisted after server restart." q2="How do I set configuration parameters with Docker?" a2="Use the `FALKORDB_ARGS` environment variable: `docker run -p 6379:6379 -e FALKORDB_ARGS='THREAD_COUNT 4 CACHE_SIZE 50' --rm falkordb/falkordb:latest`. For Redis settings like authentication, use the `REDIS_ARGS` variable: `-e REDIS_ARGS='--requirepass mypassword'`." q3="How many threads should I configure for FalkorDB?" a3="The `THREAD_COUNT` parameter defaults to your system's logical core count. This controls the maximum concurrent queries. For production workloads, tune this based on your query patterns and available CPU cores. Note that `OMP_THREAD_COUNT` separately controls parallelism *within* individual queries for GraphBLAS computations." q4="How do I limit query memory usage and execution time?" a4="Use `QUERY_MEM_CAPACITY` to set a memory limit (in bytes) per query, and `TIMEOUT` or `TIMEOUT_DEFAULT` to set execution time limits (in milliseconds). Both can be changed at runtime with `GRAPH.CONFIG SET`. A value of 0 means unlimited." q5="How can I check the current configuration values?" a5="Use `GRAPH.CONFIG GET *` to retrieve all current configuration values, or `GRAPH.CONFIG GET PARAMETER_NAME` for a specific parameter. These commands work from any Redis client or redis-cli." %}
