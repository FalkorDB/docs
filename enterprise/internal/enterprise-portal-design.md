...
| `breadcrumbs.tsx` | `Breadcrumbs` | server-safe |
| `list-filters.tsx` | `SearchForm`, `FilterChips`, `ListToolbar` | server-safe |
| `tab-nav.tsx` | `TabNav` | `"use client"` |
| `form.tsx` | `Field`, `TextInput`, `SelectInput`, `TextArea`, `SubmitButton`, `FormFeedback`, `controlClasses` | `"use client"` |
| `action-dialog.tsx` | `ActionDialog` | `"use client"` |

Server-safe matters: those five carry no directive, so a server page composes
a whole screen — header, toolbar, panels, tables, status pills — without a
client boundary anywhere in it. Only forms, tab navigation and dialogs, which
need `useActionState`, `useFormStatus` and `usePathname`, cross over.

`list-filters.tsx` is server-safe because it filters through the URL rather
than through client state: `SearchForm` is a plain GET form and `FilterChips`
are links, so the filtering happens on the server where the whole list is, the
result is a URL somebody can paste into a ticket, and both work before
hydration.
...
### A link that was never a colour

`text-falkor-primary` was written on every in-page link in the portal and named
nothing: no Tailwind config entry and no CSS variable ever defined
`falkor-primary`, so the class compiled to an empty rule and every link rendered
as body text that happened to underline on hover. It went unnoticed because the
customer pages are mostly forms and tables of facts; the staff console is mostly
links *between records*, which is where a page of unclickable-looking prose
becomes a navigation problem rather than a cosmetic one.

It is a token now, indigo to match `--primary` but a step darker in the light
theme and much lighter in the dark one, because a link is small text on a card
rather than white text on a button. `ContactButton` was the one place using it
as a background and moved to `bg-primary`, which is what it meant.

### The staff console is a list and a record, not a table of forms

Every staff page put its controls in its table. An organization row carried a
textarea for the view-as reason, a second textarea for the suspension reason,
and the buttons for both; the quota queue carried a decision note per row; the
member list was a form per person, with "change role" one enter key away from
"remove". Twelve customers meant something like ninety live controls on one
screen, and the reason box you were typing into was identified only by which row
it happened to sit in.

Three changes, applied across `/staff`, `/staff/quota` and `/staff/releases`:

**Rows carry verbs; verbs carry their own fields.** `ActionDialog` wraps a
server action, its hidden fields, its reason box and its confirm buttons, and is
the single component behind approve/reject, suspend/reinstate, view-as, invite,
change role, password reset, removal, quota decisions and publishing. Each
dialog names the customer or the person it acts on, so a screen reader hitting a
table of them does not hear "Remove" twelve times with nothing to tell them
apart, and each states what the action does to somebody before it is confirmed.
The reason is still enforced by the service rather than by the dialog — what
changed is where it is asked for.

**Lists are filtered and searched through the URL.** Up to five hundred
organizations with no way to narrow them meant find-in-page, which cannot see
the rows past the query's cap and so is quietly wrong on a large installation.
Status chips carry their counts, which is also how "3 awaiting approval" gets
said without a second table above the first one. The quota queue opens on
`PENDING` for the same reason it used to be split in two — it is a work queue
first and a history second — but the split is now a filter, so the history is
one click away instead of unreachable.

**One customer is one record with tabs.** `/staff/organizations/[id]` has a
layout carrying the breadcrumb, the name, the status and the action bar, with
Overview, Quota and Members as routed tabs beneath it. Before, the verbs were in
the list and the facts were on the page, so suspending a customer whose page you
were reading meant going back and finding their row again; members was a third
page whose only way back was a link to all five hundred organizations.

The layout reads a summary through `loadOrganizationSummary`, which does *not*
enter the tenant. Crossing is audited in the customer's own log, and a layout
that crossed would write an entry for the frame as well as for the tab inside
it — two lines per view, one of which describes nothing. It is wrapped in
React's `cache` so the layout and its page share one query per request.

**What this costs.** A Radix dialog is scripting, and the forms it replaces
posted without any. The trade is confined to the staff console deliberately: the
customer-facing pages keep their plain forms, and what it buys is an operator
who can tell which reason box belongs to which customer.
...
**A reader with no session gets no navigation either, wherever they are.** The
path list above covers every page a signed-out visitor is *sent* to and missed
the one they can reach by typing: the 404. Every entry in the customer
navigation redirects them to sign in, so that page drew eleven dead links
beside a card asking them to sign in — the failure `STANDALONE_ROUTES` exists
to prevent, arrived at from a direction a path list cannot see, since a 404 has
an arbitrary path. So the shell also takes a `signedIn` boolean, resolved in
the root layout. That is a fact about the reader rather than a longer list of
paths, which matters because a path list can only ever be extended after
somebody finds the next hole in it.

It is the one thing the root layout resolves outside a Suspense boundary. There
is no sensible placeholder for "sidebar or no sidebar" — a shell that guessed
would move the page sideways once it found out — and it is affordable, because
`auth()` decodes a cookie rather than reading the database and the route was
already dynamic. It changes nothing but the 404: every other route either
redirects a signed-out visitor or is already standalone.
...
reads `isStaff` and picks the list — unless that staff session has a tenant
open, in which case the argument for the staff list evaporates along with the
redirects and the customer's own navigation is drawn instead.
...
`getTenantContext` throws for a staff session with no tenant open, and
`requireTenantForPage` turned every `NoTenantError` into `/signup` — so a staff
engineer who clicked the logo was asked to contact FalkorDB for access to
FalkorDB. `StaffHasNoTenantError` subclasses it, which leaves every service that
already catches `NoTenantError` behaving exactly as before while the one place
that turns errors into redirects can tell the two apart. A staff session that
*is* viewing an organization resolves a tenant like anybody else and never
reaches this branch.
...
One OIDC client, two authorize requests, differing in a single scope.
Zitadel renders whichever login policy an auth request resolves to. Without a
`urn:zitadel:iam:org:id:<id>` scope that is always the **instance** policy, so
an identity provider registered on an organization — FalkorDB's own Google
provider on the staff org, for instance — is never offered. Adding that scope
globally is not the fix: the same client serves customers, who live in their own
organizations, and pinning every authorize request to the staff org locks all of
them out. The scope therefore belongs to the door somebody came through, not to
the deployment, which is what `lib/auth/scopes.ts` exists to express:

- `/api/auth/signin`, the customer door, sends the base scopes and nothing else.
  It is not a page anybody sees: `pages.signIn` points at `/sign-in`, which asks
  for an email address and adds the customer's own organization scope once it
  knows which one to ask for.
- `/staff/sign-in` calls `signIn()` with `authorizationParams` carrying the staff
  organization's scope, built from `ZITADEL_STAFF_ORG_ID` — already configured,
  because `resolveIdentityFromClaims` compares it against the resource-owner
  claim. `requireStaffForPage` redirects an unauthenticated visitor there.

**The alternative was worse than an inconvenience, which is why this is written
down.** The other way to put a Google button in front of staff is to register the
provider on the Zitadel *instance*. A user auto-created that way is placed in the
instance's default organization, so making SSO-provisioned people staff would
mean pointing that default organization at the staff org — and from then on
anything that lands a user there without an explicit org context, a
self-registration or an unscoped external-IdP first login, is a FalkorDB staff
member. The staff boundary would rest on an instance-wide setting the portal
neither owns nor validates. Org-scoping the staff request keeps that setting
irrelevant, and **the Zitadel instance default organization must not be the
staff org**.

What the staff door does not do is grant anything. A first sign-in through it
produces `isStaff: true` with `roles: []`; every staff capability is gated on a
role granted on the portal's project, so the console is empty until somebody
assigns one. Authentication says which organization vouched for the person,
authorization is still a separate act.

The scope builder validates the organization id even though it comes from the
deployment's own configuration. Scopes are space-separated, so a value with a
space in it is not a malformed id — it is a second scope, chosen by whoever
supplied the value.

A customer's own provider, connected through `/sso`, sits on the customer's
organization and is reached the same way — see the sign-in page below, which
scopes the request to the customer's organization once it knows which one to
ask for.

### The sign-in page

`/sign-in` is the portal's own page, configured as Auth.js's `pages.signIn`. It
replaced the built-in one, whose single button read **"Sign in with Zitadel"** —
the name of a component of our infrastructure, which a customer has no reason to
recognise and which tells somebody who signs in through Okta that they are in
the wrong place.

It asks for an email address first, then routes on the answer:

1. The domain resolves to a portal `Organization`, which carries `zitadelOrgId`.
   This is the same domain-to-tenant mapping membership already runs on, so no
   new source of truth appears.
2. If the directory credentials are configured, the organization's active
   providers are listed. **Exactly one** yields
   `urn:zitadel:iam:org:idp:id:{idpId}`, which sends the visitor straight to that
   provider. More than one sends the org scope alone and lets Zitadel render its
   own chooser — partly because one button cannot express a choice, and partly
   because login v2 at v4.15.3 reads `identityProviders[0].type` instead of the
   matched provider's, so naming a provider is only safe when there is nothing
   to get wrong.
3. No provider means Zitadel's own page, carrying `login_hint`, which login v2
   auto-submits — so the address is typed once here rather than once here and
   again there.

**There is no password field, and there must not be one.** Zitadel does not
support the resource-owner password grant — it is refused on security grounds
and removed in OAuth 2.1 — so a password step in the portal would mean
reimplementing lockout, second factors and passkey enrolment against the Session
API, and holding customer passwords to do it. The design already says passwords
live in Zitadel; this page is where that would quietly have stopped being true.

**The page names no provider.** Discovery runs before anyone has authenticated,
so everything it renders is readable by typing an address into it. "acme.com is
a FalkorDB customer, and they use Okta" is a fact about a customer rather than
about the person asking, and a customer list is worth more to a competitor than
it is to the visitor — who already knows which provider they use. The screen
says "your organization uses single sign-on" and stops there.

That leaves the weaker disclosure that a domain has SSO at all, which is the
irreducible cost of an email-first page. It is bounded rather than removed: the
discovery step is the portal's only unauthenticated surface that reads the
database, and it is rate limited per client address. The limiter is in process
memory and **fails open** above a cap on tracked keys — a caller spraying source
addresses would otherwise turn a rate limiter into a memory leak, and failing
closed would let that same caller stop everyone else signing in. Availability of
sign-in is worth more than the fact it protects.

Discovery is an enhancement and never a dependency. An unreachable database, an
unreachable Zitadel, or absent directory credentials all degrade to the ordinary
login page — which is where every one of these visitors went before the page
existed.
...
for customers, so for a customer session there is no code path where the tenant
is influenced by anything the caller sends. A user who needs access to two
organizations needs two accounts.

Staff are the one exception, and it is deliberately not an exception to the
sentence above: a staff session's tenant is not *read* from the request either.
It is carried in a ticket the server itself minted, encrypted with the portal's
own key, bound to the staff member it was issued to, and expiring in an hour —
see [Viewing as an organization](#viewing-as-an-organization). An
attacker-supplied value cannot become a tenant, which is the property that
matters; a value the server issued after checking a permission and writing an
audit entry can.
...
### Viewing as an organization

Support answers questions about a portal they cannot see. "My licence is
missing", "the install command 401s", "my colleague has no access" are all
questions about one tenant's screen, and reconstructing that screen from a
database is guesswork with a customer waiting. So FalkorDB staff can open a
customer's portal as the customer sees it, from that organization's row in
`/staff` or from the action bar on the organization's own page. It is the one
verb that stayed in the list rather than moving onto the record, because it is
what a support engineer opens that page to do.

**It goes through `getTenantContext`, not around it.** The obvious shape — a
staff-only rendering path that takes an organization id — would have been a
second answer to "which tenant is this request in?", and two answers to that
question eventually disagree. Instead the ticket is resolved inside the same
function every page, action and route handler already calls, so a staff session
viewing a tenant is the ordinary path with a different organization in it. No
page, service or repository knows the difference; `TenantContext.actingAsStaff`
is set, and everything that already existed keeps working.

**The ticket is a cookie, and it is encrypted for a reason.** It is the only
input to the decision, and it lives in the one place we do not control. AES-256-
GCM through the same `secret-box` used for registry credentials means an edited
cookie fails its authentication tag and decodes to nothing, so a staff member
cannot type another customer's id into devtools and be believed. It also
records who it was issued to, because encryption alone leaves a cookie
*replayable*: lifted from one engineer's browser it would otherwise open the
same tenant in anybody else's session, still carrying the first engineer's
reason. The reader compares that against the live session, and a mismatch is
not an error — it is a ticket that does not exist.

**It expires on its own, in an hour.** Long enough to work a ticket, short
enough that a closed laptop is not still inside a customer's account tomorrow.
The expiry is inside the encrypted payload as well as in the cookie's
`Max-Age`, because `Max-Age` is a request to the browser and the browser is the
party being distrusted. `tenant:read-any` is re-checked on every request rather
than assumed from the ticket, since roles are frozen into the session and a
ticket outlives a role change by up to an hour.

**One function answers "is a tenant open?", and the permission is checked
inside it.** The tenant lookup, the banner and the sidebar all ask
`viewAsTicketFor`, so the check cannot live in each of them. It was originally
in the data path only, on the reasoning that authorization belongs there — but
the chrome reads the same ticket, and a check the UI does not share is one the
UI disagrees with: a staff member whose access had just been narrowed kept a
customer's navigation and a strip naming their organization and reason, over
pages that had stopped loading. A second copy of a check is not defence in
depth when both copies answer the same question for the same request.

**A reason is mandatory and the customer reads it.** Entering writes one
`staff.tenant.assume` entry into that customer's own audit log — actor, reason,
and the window it is valid for — and it is awaited before the cookie is set, so
a failed audit write means no visit. Actions taken inside the tenant carry the
same reason onto their own audit entries, because every service already copies
`tenant.staffReason`. One entry per visit rather than one per page view: a log
that records every navigation is a log nobody reads.

**Staff keep their own permissions inside the tenant.** A support engineer's
role is read-only, so viewing is all they can do; an account manager or super
admin can act. Narrowing everyone to reads was considered and rejected — staff
who need to fix something for a customer would be pushed to doing it out of
band, where nothing is audited at all. Visibility is the control, as it is for
registry decryption: the customer sees who acted in their tenant and why.

**Suspended organizations can be opened.** A customer whose people cannot sign
in is a customer whose tenant is in exactly that state, and the status check
that sends them to `/suspended` would otherwise hide the thing being
investigated. Staff bypass it; the banner names the status instead.

**The banner is the only thing that must not be subtle.** Everywhere else the
session is deliberately indistinguishable from the customer's own — that is the
feature. So the shell carries a strip naming the organization, its status, the
reason given, and the minutes remaining, with the control that ends the visit.
The remaining time is on it because the ticket expires by itself, and an expiry
nobody was warned about reads as "the portal logged me out". While a tenant is
open the sidebar shows the *customer's* navigation, since the argument for
giving staff a different one was that every customer destination would bounce
them out, and while a tenant is open none of them do.
...
| `account-manager` | FalkorDB's own Zitadel org | Approve or reject signups, set quota, review quota requests, publish releases, add and remove members in any customer organization, and view a customer's portal as they see it |
| `support-engineer` | FalkorDB's own Zitadel org | Read every organization's metadata, audit logs, and registry credentials, and view a customer's portal as they see it |
...
roles through the Management API, authenticating as a service user with a
personal access token (`ZITADEL_API_TOKEN`).

**That service user needs an instance-level manager role**, and it is worth
being precise about why, because the failure is misleading. Zitadel's
`POST /v2/organizations` does not check `org.create` as its proto documents — it
checks `org.write` against the id of the organization *being created*, which no
membership can match. A service user holding only organization-level roles
therefore fails with `membership not found (AUTHZ-cdgFk)`, a 404 that reads like
a missing organization rather than a permissions problem. `IAM_ORG_MANAGER` is
the narrowest role that works.

**This is the portal's largest identity-provider assumption.** Zitadel has
no create-organizations-only role, so the token carries read and write over
every organization in the instance — including those belonging to the other
applications that share it under ADR 0001. That is a recorded trade rather than
an oversight: the portal gets its own machine user, never the instance admin,
and the token is sealed per cluster.
...
#### The organization is found by domain, then created by company name

`ensureZitadelOrganization` resolves the Zitadel organization when a portal
organization is created or approved, and writes the id back to `zitadelOrgId`.
Until that field is set the tenant is not usable in any sense: every membership
call begins by checking it, and a customer signing in resolves to no portal
organization and lands on `/signup`.

**Lookup precedes create, and the key is the email domain.** ADR 0001 settles
what an organization means on the shared instance: a company, not a company's
use of one product. So the organization may already exist because FalkorDB Cloud
onboarded the same customer first, and the only key both applications can agree
on is the verified email domain — names are mutable and prefixed differently per
product. Provisioning therefore searches every domain on the portal
organization, instance-wide, and joins what it finds.

`domainQuery` matches **verified** domains only; Zitadel maps it to a subselect
that hard-codes `is_verified = true`. That is the property this needs, because
an unverified claim is an assertion nobody checked and joining a tenant to one
would be trusting whoever typed it. Matching is exact and case-insensitive
rather than any substring method: every organization also carries a generated
verified domain of `<name>.<instance-domain>`, so `STARTS_WITH acme` would match
`acme.example` belonging to somebody else.

When nothing owns the domains, the organization is created named after the
**company** — no product prefix — and the domains are then claimed onto it. The
claim is not optional: a domain nobody claimed is a domain the next
application's lookup cannot find, which produces exactly the duplicate the
contract exists to prevent.

Three failures are refused rather than guessed at, all for the same reason:

- **Two organizations own the customer's domains.** That is a company split
  across two tenants. Binding to either half makes the split permanent and
  invisible, so it stops and names both.
- **The name is taken and the domain lookup did not explain it.** Either a
  different company shares the name, or it is our own provision that failed
  before claiming its domains. Those need opposite fixes and nothing here can
  tell them apart.
- **The domain was claimed between the lookup and the write.** A race the lookup
  could not have seen; nothing is recorded, because an organization bound
  without its domain is one the next application will duplicate.

The name-taken path is where a half-finished provision is recovered. Creating is
two writes — the organization, then its domains — followed by a third to record
the id here, and a crash between any of them once left an organization nobody
could find again. So the organization is created carrying **the portal's own
organization id** rather than one Zitadel generates. Zitadel treats
caller-supplied ids as a first-class feature (any string of 1–200 characters;
the proto's own example is a UUID), which makes the orphan addressable: the
retry asks whether an organization with that id exists and adopts it on proof.
That is the one name collision it is safe to resolve automatically, and it is
the only case where the answer does not depend on a name at all.

Failing that, the lookup is retried a few times before giving up, for the window
between a competing create and its claim: two staff members provisioning the
same customer at once means the loser's 409 arrives when the winner's *create*
commits, which is before the winner has claimed any domain. The retry is
bounded — a staff member is waiting on a form, so the choice is between a short
wait and a clear refusal they can act on.

**It deliberately no longer adopts an organization found by name.** That was
sound while the portal owned its instance — every organization whose name it
could collide with was one it had created. On a shared instance the same code
binds a customer's tenant to a stranger's organization whenever two companies
share a name. `zitadelOrgId`'s unique sparse index and `setZitadelOrgId`'s
refusal to rebind remain the backstops.

Whether a claimed domain needs a DNS challenge depends on the instance's
`ValidateOrgDomains` policy. With it off — Zitadel's default, and the dev
instance's setting — the domain is verified as it is added and is discoverable
at once. With it on, the domain is added unverified and someone must complete
the challenge before other applications can find the organization by it;
provisioning does not try to automate that, because an unverified claim is not
a claim.

### An organization is not usable until the project is granted to it

Provisioning created the customer's organization and stopped. That is not
enough, and the gap was invisible for a long time.

Roles in Zitadel are not properties of a user. They are keys declared on a
**project**, handed to a person through a **user grant**. The portal has one
project — `ZITADEL_PROJECT_ID` — owned by FalkorDB's own organization, and each
customer lives in an organization of their own. Zitadel will not let an
organization hand out roles from somebody else's project unless that project has
been **granted** to it, and a user grant against a granted project has to name
the grant it is exercising. So provisioning was producing organizations nobody
could be invited into: the first invitation answered

```
POST /management/v1/users/{id}/grants -> 400
```

naming neither the project nor the missing grant.

**The test suite agreed with the bug.** Every case in `directory.test.ts` built
the project *inside* the customer's organization, which is the one arrangement
where a bare `projectId` is sufficient — and an arrangement production never
has, since the project is shared by definition. The suite passed while the
feature could not work. `a customer organization in the production shape` now
builds it the other way and was confirmed to fail against the code that shipped
without it.

So `provisionUsableOrg` does three things, all idempotent:

1. **declares the role keys on the project** — `org-admin`, `developer`,
   `viewer`. Once-ever work rather than a migration, because the portal is
   deployed against instances it did not set up and an install missing a key
   should heal on the next onboarding instead of failing every invitation until
   somebody reads the release notes;
2. **grants the project to the customer's organization**, carrying all three
   keys — which role a person gets is the customer's decision, made on their own
   members page;
3. leaves the user grants to `grantRole`, which now names the project grant when
   the project belongs elsewhere and omits it when it does not.

**Both run before `setZitadelOrgId`.** Recording the id first and failing here
would leave an organization the portal believes is provisioned and cannot use —
and `ensureZitadelOrganization` returns early on exactly that field, so the
retry it is supposed to be would skip the part that failed, forever.

**Organizations provisioned before this have an id and no grant**, so two paths
repair them rather than requiring a migration: `provisionIdentity` no longer
returns early when the id is set, and `inviteMember` ensures the grant before it
grants a role — which is the moment the omission would otherwise surface, as a
400 that reads like a bad request.

### Staff-managed membership

Creating an organization was only half of onboarding. The other half —
somebody being *in* it — had no route at all: `/members` resolves the tenant
from the session, and a staff session deliberately has none, so staff were
redirected away from the only page that could add anybody. A customer's first
administrator therefore had to be created in the Zitadel console by hand.

`/staff/organizations/[id]/members` closes that, and does it by reusing the
customer-facing functions unchanged rather than growing a staff-shaped copy of
them. `inviteMember`, `listMembers`, `changeMemberRole`, `removeMember` and
`resetMemberPassword` already take a `TenantContext`, already check permissions,
and already write audit entries carrying `staffReason`. A second implementation
would be a second set of rules to keep in step — and these are the rules that
stop an organization losing its last administrator.

The way in is `assumeTenantAsStaff`, which demands a written reason of at least
eight characters and records the crossing *before* returning a context. Every
mutation carries its own reason box rather than sharing one — each is inside
that action's own dialog — so a justification typed for one action cannot be
submitted with another. All of it lands in the customer's own audit view:
FalkorDB can administer a customer's people, and the customer can see exactly
when we did and why.

It is a tab on the organization rather than a page hanging off the list, so the
customer's name, status and actions stay in view while their people are being
changed.

**`member:manage-any` is a staff permission that is translated, not granted
directly.** The services ask for `member:invite` and `member:remove` — the same
permissions an org admin holds over their own tenant — so entering a tenant maps
the cross-tenant permission onto them. Granting `member:invite` to a staff role
outright would grant it in every context that reads a staff session, which is a
much larger claim than "may administer a customer's membership after declaring
why". It is held by `super-admin` and `account-manager`, and withheld from
`support-engineer`, whose access is deliberately read-only.

**`assumeTenantAsStaff` carries the customer's Zitadel organization**, read from
the organization being entered. It previously carried `session.zitadelOrgId` —
FalkorDB's own — and nothing noticed, because its only caller was the quota path
and quota never reads the field. Membership does: it is the organization a user
is created in. Left alone, adding a customer's first administrator would have
created that person inside FalkorDB's staff organization, where
`resolveIdentityFromClaims` calls everybody staff.
...
`Cluster.status` is `PENDING_QUOTA | QUOTA_REFUSED | ACTIVE | DECOMMISSIONED`.
Neither "awaiting binding" nor "expired" is among them — see
[Two statuses that are not stored](#two-statuses-that-are-not-stored).
...
- **`Cluster`** — the unit customers register and the unit quota is measured
  ...
  (`cluster:update`, audited as `cluster.update`). `status` moves between
  service and retirement under a separate permission (`cluster:decommission`,
  audited as `cluster.retire` and `cluster.reactivate`) — see
  [Retiring never returns capacity](#retiring-never-returns-capacity). Nothing
  else on the record is editable: capacity is quota-bearing and `k8sClusterId`
  is what licenses are bound to.
...
- **The search is scoped to the organization.** Zitadel makes email addresses
  ...
  user.
- **Somebody already set up cannot be invited, and that is not a failure.**
  Zitadel refuses to mint an invite code for a user who already has a password,
  a passkey or a federated login — the code exists to establish a first
  authentication method and there is nothing left to establish. It refuses with
  a bare `400`, so `sendInvite` matches its error id (`COMMAND-EF34g`) and
  returns `already-enrolled` rather than throwing.

  Returning it matters more than reporting it. The ordinary way to reach this
  state is re-inviting somebody who was **removed**: removal revokes their
  grants and leaves their account standing, so Zitadel is guaranteed to refuse.
  Abandoning the invitation there would skip the access revocation being lifted
  — leaving a person Zitadel says has access and the portal bounces to
  `/no-access` on every request, with nothing in the interface to undo it. So
  the outcome is carried to the end: the role is granted, the revocation is
  lifted, the invitation is recorded, and the audit entry says `emailSent:
  false`. The form then says no email was sent and why, instead of claiming one
  was.

### Where the emails lead

An invitation is only as good as the page its link opens, and both halves of
that were wrong by default.

**The link is spelled out, not left to Zitadel.** Zitadel builds a default link
for every notification, and for an invitation that default points at the
*legacy* login application (`/ui/login/user/invite`) whatever the instance
serves — invitations are the one flow absent from Zitadel's `Login.DefaultPaths`,
so unlike password resets and email verifications the default cannot follow the
instance. On a deployment serving the current login application that link is a
page the recipient cannot complete: the invitation arrives and the account
cannot be set up. So `sendInvite` and `sendPasswordReset` both pass an explicit
`urlTemplate` naming `/ui/v2/login`, with the `{{.UserID}}`, `{{.OrgID}}` and
`{{.Code}}` placeholders Zitadel substitutes when it renders the mail. Zitadel
caps that field at 200 characters and rejects a longer one outright, so an
instance domain long enough to breach the cap drops the field rather than
losing the email.

**The flow ends on the portal, through the instance's `defaultRedirectUri`.** A
link opened from a mailbox starts a flow with no application behind it, so when
the login application is finished it has nowhere to send anybody and shows its
own "you are signed in" page — the identity provider, which the recipient has
never heard of, with no way on to the product they were invited to.
`ensureLoginRedirect` sets `defaultRedirectUri` on the *instance* login policy
to the portal's origin, taken from `AUTH_URL`, and both login applications
honour it at the end of the invitation, verification and password flows.

Three properties of that, in the order they matter:

- **The instance, not the organization.** Every organization inherits it,
  including ones provisioned before this existed and ones created directly in
  Zitadel. Writing an organization-level policy to carry one field would freeze
  that organization's registration, MFA and session settings at whatever the
  instance said on the day somebody was invited — the policy is inherited whole
  or not at all.
- **Best-effort, and never the reason an email fails.** The endpoint needs
  `iam.policy.write`, which an install running the portal with `IAM_ORG_MANAGER`
  does not have. A refusal is logged with what to grant and the invitation
  proceeds: the recipient still gets the mail and still sets a password, and
  merely has to find the portal themselves.
- **Read before written, and cached after.** `PUT` replaces the whole policy, so
  the write carries back the fields the read returned; inventing them would turn
  an invitation into a silent reset of the instance's MFA rules, and omitting the
  lifetimes would expire every session in it. A read that returns *no* policy is
  the case that must not proceed at all — there is nothing to send back, so the
  write would blank the instance's login settings on the way to setting one
  field. That outcome is not cached, so an instance that answers properly later
  is written to then. Everything else is memoized per process, so a resend costs
  nothing.

This is a fallback, and it is one because an authentication request names the
application that started the flow and takes precedence over it. When a second
application shares the instance, that — not a per-organization redirect — is
...
- **A signed key is readable again, and the read is audited.** The portal used
  to show a key exactly once, at issuance. That made an install command
  unrecoverable, so the answer to a lost values file was to rotate a working
  credential — and because verification is offline, rotating leaves the customer
  holding two live keys and the portal listing one. Recovering by rotating was
  solving a copying problem with a credential change. `revealLicenseKey` now
  hands the key back on the same terms as the registry token: gated on
  `license:issue`, and every read written to the customer's audit log as
  `license.reveal`. It does not widen who can obtain a key — anyone who could
  rotate one could already obtain one — it makes obtaining one recorded and
  free. Revoked and expired keys are refused, so recovery cannot rebuild an
  install around a key the portal has stopped counting.
...
Requiring `k8sClusterId` means the customer must have a **Kubernetes cluster**
before they can have a license. It does not mean they must have installed
anything: the value is the `kube-system` namespace UID, which exists from the
moment the cluster does, and `kubectl get ns kube-system -o jsonpath={.metadata.uid}`
reads it out of an empty cluster as readily as a running one.

So the order is: register, then install. The registration wizard asks for the
UID up front and issues the license before anything is deployed, which is what
lets it hand back an install command with the license already in it. The Admin
Server's **14-day trial with unlimited limits when no license is present**
remains the safety net for anyone who installs first anyway.
...
The check is `sum(registered clusters) <= quota`, and it is correct. What it
does not do is notice when the question is malformed, and every one of these
mints a signed, offline-verified license that cannot be recalled.

**A decommissioned cluster.** This one is now closed by the sum itself — see
[Retiring never returns capacity](#retiring-never-returns-capacity) — but it is
worth recording what it was, because the shape recurs. Quota used to be summed
over live clusters only, so a retired one contributed nothing to the ceiling
while its declared capacity was still sitting on its own document. Retire a
cluster, register another with the freed room, and both would hold a license:
the check passed because it was not counting the capacity the license was about
to grant. The guard in `mintLicense` remains as the other half of the answer —
the sum stops a replacement being registered, and the guard stops the retired
cluster itself being handed a fresh license after its owner declared it finished
with.
...
### A registration that does not fit is parked

A declaration larger than the agreement covers used to be refused outright: the
wizard was replaced by a contact button, and the customer was left to raise a
quota request that described the same cluster a second time, then describe it a
third time once the request was granted. Three descriptions of one cluster, two
of them retyped from memory.

It is parked instead. The cluster is written with status `PENDING_QUOTA`, a
request for exactly the capacity it needs is raised against it in the same
transaction, and approving that request licenses the cluster automatically.
Nothing is retyped, because nothing was thrown away. The request is raised on
the customer's behalf whether or not they hold `quota:request` — a developer can
register, so a developer can reach the state that needs asking.

`neededLimits` raises each dimension to cover what is already registered plus
this cluster and never lowers one, so staff read a request whose changed numbers
are the ones actually being asked for rather than a restatement of the whole
agreement.

**The ordering in `completePendingRegistration` is load-bearing.** The cluster is
activated before its license is minted, because `mintLicense` re-runs the quota
check and assumes the cluster's own capacity is already counted in the sum.
Completion also runs after the quota-raise transaction has committed, so the
approved limits stick even in the case where the cluster still does not fit.

**A parked cluster is a fifth thing the quota check does not cover.**
`registeredCapacity` excludes `PENDING_QUOTA` and must: a parked cluster holds
no license and cannot be allowed to consume the ceiling while it waits. But that
means hand-issuing a license to one would check its capacity against a sum that
is missing it, which is the decommissioned-cluster bug wearing a different hat.
`mintLicense` raises `ClusterAwaitingQuotaError` for a parked cluster, and the
cluster page hides the issue control, so the gate cannot be walked past from
either direction.

**Refusing the request has to let go of the cluster too.** `reviewQuotaRequest`
originally called `completePendingRegistration` only on approval, so a rejected
request left its cluster in `PENDING_QUOTA` for good — on a page promising it
would be licensed automatically once the request was approved, about a request
staff had just turned down. A rejection now moves it to `QUOTA_REFUSED`, which
is terminal: `mintLicense` raises `ClusterQuotaRefusedError`, `retireCluster`
refuses it, and the cluster page explains what happened and links to the
request.

`QUOTA_REFUSED` is *not* `DECOMMISSIONED`, which counts towards capacity. This
cluster was never licensed and never ran, so charging its declared capacity
against the agreement would bill for something that never existed — and would
eat the room a corrected request needs. Both uncounted statuses are named once,
in `UNCOUNTED_CLUSTER_STATUS`, because capacity sums, licensing and retirement
all have to agree on the list and three copies eventually disagree.

### Retiring never returns capacity

Retiring a cluster records that an install is finished with. It does not give
its declared capacity back, and the portal says so in the confirmation, in the
cluster list and on the capacity page.

The reason is the same one that runs through the rest of licensing: we cannot
take a license back. They are signed and verified offline inside the customer's
own cluster, so a retired install whose license has not expired can still be
started and will still work. If retiring returned its capacity, an organization
could register, retire, register again, and hold two valid licenses for capacity
sold once — repeatable without limit and undetectable from here. So
`registeredCapacity` counts `DECOMMISSIONED` clusters, and the loop is closed by
arithmetic rather than by a rule somebody has to remember.

**Rejected: releasing capacity when the cluster's last license expires.** It has
the same anti-churn property — each cycle would cost a full license term — and
it would spare customers who rebuild clusters legitimately, which is commoner
than it sounds because recreating a cluster changes its kube-system UID and so
produces a new registration. It was rejected as a second rule to reason about
for a case that a quota increase already answers. Worth revisiting if rebuild
traffic turns out to be routine.

**Which makes retirement reversible for free.** A retired cluster consumes
exactly what an active one does, so putting one back spends nothing and needs no
capacity check. That is deliberate: the customer can undo their own mistake
instead of asking us, and an action that cannot cost anything does not need a
frightening confirmation. `setRetired` restores `ACTIVE`, and a cluster that was
never bound still describes itself as awaiting binding — see
[Two statuses that are not stored](#two-statuses-that-are-not-stored).

**The consequence for the UI is that retired clusters must stay visible.** They
were hidden while retirement returned their capacity, which made hiding them
harmless. Now that they keep consuming it, a cluster list that leaves them out
is a list that cannot account for the number on the capacity page — and a total
the customer cannot reconcile against anything is a support ticket. So
`/clusters` shows them in a second table, the staff console reads every cluster
rather than the live ones, and each quota meter draws the held portion as its
own segment with the amount named beside it.

### Two statuses that are not stored

`Cluster.status` says where a cluster is in its life and nothing else:
`PENDING_QUOTA | QUOTA_REFUSED | ACTIVE | DECOMMISSIONED`. Each value changes
because a person did something. Two facts customers read off the same row are
derived instead, by `clusterState`.

**"Awaiting binding" used to be a status.** It meant "no license can be bound to
this yet", which is true exactly when `k8sClusterId` is absent — `mintLicense`
raises `ClusterNotBoundError` on that field, not on the status. So the document
already carried the fact and the stored copy was a second answer to the same
question, free to disagree with the one that actually decided anything. It is
computed now: an `ACTIVE` cluster with no UID reports `AWAITING_BINDING`, and
only `ACTIVE` is reinterpreted, because what happened to a pending, refused or
retired cluster matters more than a binding it was never going to use. The
schema default moved to `ACTIVE`, and `worker backfill-cluster-status` moves
documents written before the change — they are not harmless, since `setRetired`
matches on `ACTIVE` and a legacy row would refuse to retire with "this cluster
is already retired".

**Expiry was never a status and should not become one.** It belongs to the
license rather than the cluster; it is not exclusive with any lifecycle value,
since a decommissioned cluster's key expires too; and nothing would flip it —
expiry happens at an instant with no actor, so a stored `EXPIRED` would say
whatever a sweeper last left behind. Telling a customer they are licensed while
their cluster says otherwise is the worst thing this field could do, so expiry
is compared against the clock at the point of use and shown in its own column.

The rule underneath both: a status is for what someone did, not for what is
merely true. Anything derivable from a field the document already holds gets
derived, so the label cannot drift from the behaviour it describes. Nothing in
`clusterState` decides whether an action is allowed — licensing, retirement and
capacity read `status` directly, because a label is not a permission.
...
no quota, or a lapsed term on the licensing page; no permission or no quota on
the clusters page. Registration additionally shows what is left of each limit
beside the field that spends it, because the quota is checked against the sum of
every live cluster and the only number that predicts the outcome is the
remainder — the number the customer did not have.

Exhausted headroom used to be on that list and no longer is. Once a registration
that does not fit is [parked rather than refused](#a-registration-that-does-not-fit-is-parked),
replacing the wizard with a contact button would have blocked the one path built
for exactly that case, and the copy it showed advised decommissioning a cluster
to free the room — which is not what decommissioning does, then or now. It is a
warning on the capacity and review steps instead: what the agreement does not
cover, and what registering anyway will do. For the same reason the declared
size is no longer capped at the headroom. A number the agreement does not cover
is a request for that number, not a typo.

The expiry form still constrains what it accepts — it cannot be set before today
or after the agreement ends. That is courtesy, not control. Every one of these
limits is checked again in the action, because a form is a suggestion and the
...
One credential per organization made every pull secret the same secret. A leak
meant rotating the whole estate's CI at once, and retiring an install could not
stop it pulling, because the thing it pulled with belonged to everybody. Each
cluster now holds its own token, minted when it is registered.

**One machine user, many tokens.** Gitea tokens belong to a user but are revoked
individually, so the per-organization machine user can hold a token per cluster
without provisioning a Gitea account per cluster. That is also the trap in
`credentials.ts`, and it runs in **both** directions. `issueAndStore` used to
delete *every* token the user held before minting, which was right while there
was one credential and became a way for a customer pressing "replace" on the
organization token to stop the whole estate pulling — under a button whose copy
promises it affects only what uses that token. Both mints now delete exactly the
one id they are replacing. Two integration tests pin it, one per direction, and
both were confirmed to fail against the code that shipped without them.

**The machine user is recorded the first time it is used**, not the first time
the *organization* is given a credential. Every revocation path finds the user
through `organization.giteaUsername`, and a cluster token minted while that was
unset — reachable because the organization mint is best-effort and swallows its
errors — was a credential suspension silently failed to revoke while reporting
success. `revokeClusterRegistryCredential` now refuses to forget a token id it
could not delete, because that id is the only handle the portal has on it.

**The organization token stayed.** It is what pulls that belong to no cluster
use — CI that mirrors images, an air-gapped sync — which own no `kube-system`
UID and would otherwise have to borrow some cluster's token and break when that
cluster was retired. It is also what every customer provisioned before this
change already has in their pull secrets, so removing it would have been a
breaking change on somebody else's schedule.

It has no page of its own any more, and neither does a cluster's own token: both
live in that cluster's install card, beside the Secret the value goes into. A
cluster with no token of its own says so there and offers to mint one, and until
it does its pull secret is filled from the organization credential, so nothing
stops pulling while it waits. Replacing a token rewrites the generated Secret in
place — the customer applies the file that is already on screen rather than
discovering that the one they copied a moment ago is now the old one.
`/access-token` and `/registry` both redirect to `/clusters`.

### What ends a cluster's token, and what deliberately does not

Retirement and suspension end it. Both are decisions somebody makes, and both
already meant "this should stop pulling".

Retiring therefore has a consequence it did not have before: reactivating mints
a **new** token, because revocation deleted the old value and a deleted Gitea
token cannot come back. Retirement is otherwise designed to be free and
reversible, so the cluster page says this rather than leaving it to be found as
a failing pull.

**License expiry does not.** It is tempting — an expired license should not keep
downloading new versions — and it is the wrong mechanism. A `read:package` token
cannot distinguish pulling a *newer* version, which is the thing to stop, from
re-pulling the version the customer is licensed for, which is how Kubernetes
recovers when a pod is rescheduled onto a node without a cached image. Revoking
on expiry would convert a lapsed license into an outage at the next reschedule,
to enforce offline-verified expiry that the signed payload already enforces
inside the customer's own cluster. `scanLicenseExpiry` warns at 30, 7 and 0 days
and touches no credential.

A cluster that never ran gets nothing: `PENDING_QUOTA` and `QUOTA_REFUSED` are
excluded from the listing and refused by `replaceClusterRegistryCredential`.
Minting for the second of those would hand pull access to a registration that
was declined.
...
The installer produces artifacts for [scripts/install.sh](../../scripts/install.sh)
and the chart: a `values.yaml` preview and the corresponding install command.

**It lives on the cluster page, not on a page of its own.** It used to be
`/install`, reached from the nav, which meant a customer who had just registered
a cluster was sent to a form asking them to retype the license the portal had
issued them a second earlier and the registry credentials it already held. An
installer that does not know which cluster it is for cannot fill anything in.
Registration redirects to the cluster page, and the installer is there —
expanded while the cluster has never reported a version, collapsed afterwards,
so the same section is the first install and the later recovery.
...
So `/clusters/diff` compares two pasted documents. The page shows the exact
...
The `-chart` suffix on `Chart.yaml`'s `name` is not decoration. `helm push`
appends the chart name to the OCI repository it is given, and the chart and the
portal image are both published to the `falkordb-internal` organization on
`registry.falkordb.cloud`, at the same version — so an unsuffixed chart
resolves to precisely the repository and tag the image was pushed to minutes
earlier, and takes it. `portal-v1.0.0` did exactly that, which broke the
chart's own default `image.tag` fallback to `appVersion`.

The suffix names the published artifact and nothing else. The directory stays
`helm/falkordb-enterprise-portal`, and `portal.name` is a literal rather than
`.Chart.Name`, so the suffix never reaches `app.kubernetes.io/name` — which is
part of the immutable selectors of a running Deployment and StatefulSet, where
a rename is not an upgrade any existing release could apply.
