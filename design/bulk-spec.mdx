---
title: "GRAPH.BULK Endpoint Specification for High-Speed Ingestion"
sidebarTitle: "Bulk Endpoint Spec"
description: "Technical specification for the GRAPH.BULK endpoint used to import large datasets into FalkorDB with binary data format for nodes, edges, and properties."
---
...
<AccordionGroup>
  <Accordion title="What is the GRAPH.BULK endpoint used for?">
    The GRAPH.BULK endpoint is used to **import large datasets** into FalkorDB. It accepts binary data blobs that are unpacked to create nodes, edges, and their properties, making it ideal for building custom import tools.
  </Accordion>
  <Accordion title="Can GRAPH.BULK update an existing graph?">
    No. The GRAPH.BULK endpoint can only **create new graphs**, not update existing ones. The first query in a sequence must include the `BEGIN` token, which verifies that the graph key is unused.
  </Accordion>
  <Accordion title="Why do large imports need to be sent incrementally?">
    Redis has a maximum string length of **512 megabytes** and a default maximum query size of **1 gigabyte**. Large datasets must be split across multiple GRAPH.BULK calls to stay within these limits.
  </Accordion>
  <Accordion title="How are node IDs assigned during bulk import?">
    Nodes do not specify their own IDs. Each node receives an **8-byte unsigned integer ID** corresponding to the node creation count — the first node gets ID 0, the second gets ID 1, and so on.
  </Accordion>
  <Accordion title="What property types are supported in the binary blob format?">
    The bulk format supports: `BI_NULL` (0), `BI_BOOL` (1), `BI_DOUBLE` (2), `BI_STRING` (3), `BI_LONG` (4), and `BI_ARRAY` (5). Arrays contain N values of the same type-property pair format.
  </Accordion>
</AccordionGroup>
