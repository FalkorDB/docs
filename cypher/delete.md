...
{% include faq_accordion.html
  ...
  q1="What happens to relationships when I delete a node?"
  a1="Deleting a node **automatically deletes** all of its incoming and outgoing relationships. FalkorDB does not allow orphaned relationships in the graph."
  q2="What is the difference between DELETE and DETACH DELETE?"
  a2="In FalkorDB, DELETE actually implements DETACH DELETE behavior — relationships are automatically removed when their endpoint nodes are deleted. Both keywords work the same way."
  q3="How do I delete all data in a graph?"
  a3="Use `MATCH (n) DETACH DELETE n` to remove all nodes and their relationships. Alternatively, use the `GRAPH.DELETE` command to remove the entire graph key."
  q4="Can I delete specific relationships without deleting nodes?"
  a4="Yes. Match the relationship with an alias and delete only it: `MATCH (a)-[r:KNOWS]->(b) DELETE r`. The nodes remain intact."
%}
