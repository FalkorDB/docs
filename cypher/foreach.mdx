---
title: "FOREACH Clause: Updating Collections in Cypher"
sidebarTitle: "FOREACH"
description: "The FOREACH clause feeds the components of a list to a sub-query comprised of updating clauses only"
---
...
<AccordionGroup>
  <Accordion title="What clauses can I use inside FOREACH?">
    FOREACH only supports **updating clauses**: CREATE, MERGE, SET, REMOVE, DELETE, and nested FOREACH. Read clauses like MATCH and RETURN are not allowed inside FOREACH.
  </Accordion>
  <Accordion title="Can FOREACH access variables from the outer query?">
    Yes. The sub-query inside FOREACH can read variables defined before the FOREACH clause. However, variables created inside FOREACH are local and not visible to later clauses.
  </Accordion>
  <Accordion title="How is FOREACH different from UNWIND?">
    **UNWIND** creates new rows for each list element and allows any clause. **FOREACH** iterates over a list to perform side-effect updates without changing the number of result rows.
  </Accordion>
  <Accordion title="Can I use FOREACH for conditional updates?">
    Yes. Use a CASE expression to produce a list with one element (execute) or empty list (skip): `FOREACH(x IN CASE WHEN condition THEN [1] ELSE [] END | SET n.flag = true)`.
  </Accordion>
</AccordionGroup>
