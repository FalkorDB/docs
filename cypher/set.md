...
{% include faq_accordion.html title="Frequently Asked Questions" q1="What is the difference between SET n = map and SET n += map?" a1="The `=` operator **replaces all** existing properties with the map values. The `+=` operator **merges** properties, updating only specified keys while keeping other existing properties intact." q2="Can I remove a property using SET?" a2="Yes. Setting a property to NULL removes it: `SET n.name = NULL`. This is equivalent to using the REMOVE clause." q3="Can I copy properties from one node to another?" a3="Yes. Use `MATCH (a), (b) SET a = b` to copy all of `b`s properties to `a`, replacing any existing properties on `a`." q4="Can I SET multiple properties at once?" a4="Yes. Separate property assignments with commas: `SET n.age = 33, n.name = 'Bob'`. All assignments are applied in a single operation." %}
