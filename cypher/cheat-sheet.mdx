...
// CASE — simple and generic form
RETURN CASE n.title WHEN 'Engineer' THEN 100 ELSE 0 END
...
CREATE INDEX ON :Person(age)                            // legacy syntax
...
`Index Scan` instead of `Label Scan`. Not-equal (`<>`) filters are not optimized.
...
Store vectors with `vecf32()` and search them with the vector index procedures:

```cypher
CREATE (:Product {name: 'Laptop', embedding: vecf32([0.12, 0.83, 0.44])})

CALL db.idx.vector.queryNodes('Product', 'embedding', 5, vecf32([0.15, 0.81, 0.40]))
YIELD node, score
RETURN node.name, score ORDER BY score

CALL db.idx.vector.queryRelationships('Call', 'summary', 5, vecf32([0.1, 0.2, 0.3]))
...
```cypher
RETURN vec.cosineDistance(a.embedding, b.embedding)
RETURN vec.euclideanDistance(a.embedding, b.embedding)
```
...
requires an existing range index on the same properties.
...
<AccordionGroup>
  ...
  <Accordion title="How do I check whether my query uses an index?">
    ...
    and `Label Scan` when it is not. `GRAPH.PROFILE` adds per-operation timings.
  </Accordion>
  ...
  <Accordion title="Does FalkorDB support Neo4j Cypher syntax?">
    ...
    expressions, regular-expression operators, and temporal arithmetic functions are not
    available. See [Cypher coverage](/cypher/cypher-support).
  </Accordion>
