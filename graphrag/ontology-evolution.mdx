...
If you're new to the ontology, start with [Graph Schema](/graphrag/graph-schema). This page is about *changing* an existing ontology. If you don't have one yet, see [Ontology Discovery](/graphrag/ontology-discovery) — `Ontology.from_sources` drafts one from a corpus and `GraphRAG.suggest_schema_extensions` proposes additions as new docs arrive.
