...
`MENTIONED_IN` edges are what let retrieval trace an entity mentioned in an answer back to its source chunk — see [Reducing LLM Hallucinations](/graphrag/reducing-llm-hallucinations) for how to surface that provenance trail and define an abstention path when it's missing.
...
- [Ingestion Pipeline](/graphrag/ingestion) — the 9-step pipeline from document to knowledge graph
- [Extraction](/graphrag/extraction) — the 2-step hybrid extraction process (NER + LLM)
- [Graph Schema](/graphrag/graph-schema) — how the knowledge graph is structured in FalkorDB
- [Storage](/graphrag/storage) — GraphStore, VectorStore, and EntityDeduplicator internals
- [Retrieval](/graphrag/retrieval) — the multi-path retrieval system that answers questions
- [Reducing LLM Hallucinations](/graphrag/reducing-llm-hallucinations) — grounding answers in retrieved evidence and defining an abstention path
