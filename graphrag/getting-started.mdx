...
`ConnectionConfig` accepts additional parameters such as `port`, `username`, `password`, and `query_timeout_ms`. See [Configuration](/graphrag/configuration) for the full list.

> **Alternative providers:** The SDK also exports `OpenRouterLLM` and `OpenRouterEmbedder` for use with OpenRouter. See [Configuration](/graphrag/configuration) for details.
...
- [Configuration](/graphrag/configuration) -- Tuning connection settings, chunking parameters, and retrieval options.
- [Strategies](/graphrag/strategies) -- Custom extraction and resolution strategies.
- [Benchmark](/graphrag/benchmark) -- Reproducing benchmark results on the GraphRAG-Bench Novel corpus (20 novels, 2,010 questions).
