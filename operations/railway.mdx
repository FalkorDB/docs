---
title: "Deploying FalkorDB on Railway Cloud"
...
<AccordionGroup>
  <Accordion title="How long does it take to deploy FalkorDB on Railway?">
  ...
  <Accordion title="What is the difference between FALKORDB_PRIVATE_URL and FALKORDB_PUBLIC_URL?">
  ...
  <Accordion title="Does my data persist across Railway redeployments?">
  ...
  <Accordion title="Can I deploy a FalkorDB cluster on Railway?">
    Yes. Use the [FalkorDB Cluster template](https://railway.com/deploy/falkordb-cluster) which provisions multiple nodes. You need a cluster-aware client to handle `MOVED` redirects.
  </Accordion>
  <Accordion title="How do I connect from my local machine to Railway FalkorDB?">
