---
title: "Managing Persistent ACL Users and Permissions on Docker"
sidebarTitle: "ACL Persistence"
...
<AccordionGroup>
  <Accordion title="Why do my ACL users disappear after restarting the container?">
  ...
  <Accordion title="Do I need to create the ACL file before starting the container?">
  ...
  <Accordion title="Can I mix inline ACL users in redis.conf with an ACL file?">
    No. When `aclfile` is set, you cannot also define `user` directives in `redis.conf`. Choose one mechanism for managing ACL users.
  </Accordion>
  <Accordion title="How do I secure the ACL file on the host?">
  ...
  <Accordion title="What happens if I forget to run ACL SAVE?">
