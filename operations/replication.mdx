---
title: "FalkorDB Replication: High Availability and Read Replicas"
...
<AccordionGroup>
  <Accordion title="Can I write to a replica node?">
    No. Replicas are read-only. All write operations must go through the master. Use `GRAPH.RO_QUERY` for read operations on replicas to prevent accidental write attempts.
  </Accordion>
  <Accordion title="How much replication lag should I expect?">
    Under normal load, replication lag is typically sub-millisecond. Under heavy write load, replicas may lag slightly. Monitor lag with the `INFO replication` command on the master.
  </Accordion>
  <Accordion title="How many replicas can I configure?">
  ...
  <Accordion title="What happens if the master goes down?">
  ...
  <Accordion title="Do replicas increase write performance?">
    No. Replicas only serve read traffic. All writes still go through the single master. To scale writes, consider using a [cluster setup](/operations/cluster) to distribute graphs across multiple masters.
  </Accordion>
</AccordionGroup>
