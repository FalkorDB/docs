---
title: "ACL: FalkorDB Access Control List Command Reference"
sidebarTitle: "ACL"
description: "Managing Access Control Lists, enabling administrators to control user permissions at a granular level"
---
...
<AccordionGroup>
  <Accordion title="How do I persist ACL users across server restarts?">
    Use `ACL SAVE` after configuring users with `ACL SETUSER`. You must also configure an ACL file in your server settings. See the [ACL Persistence on Docker](/operations/durability/acl-persistence) guide for details.
  </Accordion>
  <Accordion title="What is the difference between the ~ and %RW~ key patterns?">
    The `~<pattern>` rule grants full read-write access to matching graphs. The `%<permission>~<pattern>` syntax allows fine-grained control: `%R~` for read-only, `%W~` for write-only, or `%RW~` for full access (equivalent to `~`).
  </Accordion>
  <Accordion title="Can I restrict a user to only run read-only queries?">
    Yes. Grant only the `GRAPH.RO_QUERY` command and use the `%R~*` permission pattern: `ACL SETUSER reader on >pass +GRAPH.RO_QUERY %R~*`
  </Accordion>
  <Accordion title="What happens if a user tries to execute a command they do not have permission for?">
    The server returns a permission denied error, and the operation is logged in the ACL log (viewable with `ACL LOG`).
  </Accordion>
  <Accordion title="Can I grant access to specific graphs only?">
    Yes. Use key patterns like `~mygraph` for a specific graph or `~app1:*` for all graphs matching a prefix pattern.
  </Accordion>
</AccordionGroup>
