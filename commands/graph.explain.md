...
{% include faq_accordion.html title="Frequently Asked Questions" q1="Does GRAPH.EXPLAIN actually execute the query?" a1="No. `GRAPH.EXPLAIN` only constructs the execution plan without running the query. No data is read or modified. Use `GRAPH.PROFILE` if you want to execute the query and see runtime metrics." q2="What can I learn from the execution plan?" a2="The execution plan shows the sequence of operations (scans, filters, traversals, projections) the engine will perform. This helps you understand whether indexes are being used and identify potential performance bottlenecks." q3="How do I know if my query is using an index?" a3="In the execution plan output, look for operations like `Index Scan` instead of `Node By Label Scan`. An index scan indicates the query is leveraging an index for faster lookups." q4="Can I use GRAPH.EXPLAIN with parameterized queries?" a4="Yes. You can pass parameterized queries to `GRAPH.EXPLAIN` using the same `CYPHER param=val` syntax as `GRAPH.QUERY`." %}
