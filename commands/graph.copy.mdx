---
title: "GRAPH.COPY: Duplicating Graphs in FalkorDB"
sidebarTitle: "GRAPH.COPY"
...
<AccordionGroup>
  <Accordion title="Can I still query the source graph while GRAPH.COPY is running?">
    Yes. The source graph remains **fully accessible** for both reads and writes during the copy operation.
  </Accordion>
  <Accordion title="Does GRAPH.COPY also copy indexes and constraints?">
  ...
  <Accordion title="What happens if the destination graph already exists?">
  ...
  <Accordion title="Is GRAPH.COPY atomic?">
