...
<AccordionGroup>
  <Accordion title="How much does the Startup Tier cost?">
    The Startup Tier starts at **$73/month** for 1GB of RAM. Pricing is calculated at **$0.100 per Memory GB/Hour**. You can estimate costs by multiplying your memory allocation (in GB) by $73.
  </Accordion>
  <Accordion title="Does the Startup Tier include backups?">
    Yes, the Startup Tier includes **automated backups every 12 hours**, ensuring your data is regularly protected without manual intervention.
  </Accordion>
  <Accordion title="Is TLS encryption included?">
    Yes, **TLS** is included in the Startup Tier, encrypting all data transmitted between your application and FalkorDB for secure production workloads.
  </Accordion>
  <Accordion title="Can I run multiple graphs on the Startup Tier?">
    Yes, the Startup Tier supports **Multi-Graph / Multi-Tenancy**, allowing you to run multiple isolated graph databases within a single instance.
  </Accordion>
  <Accordion title="What is the maximum dataset size I can store?">
    The max graph dataset size is approximately **75% of your instance RAM**. For example, a 1GB instance supports ~0.75GB of graph data, and a 2GB instance supports ~1.5GB. Use the [graph size calculator](https://www.falkordb.com/graph-database-graph-size-calculator/) for estimates.
  </Accordion>
</AccordionGroup>
