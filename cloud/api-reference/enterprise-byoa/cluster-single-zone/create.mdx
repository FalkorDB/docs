...
Provisions a new Enterprise BYOA Cluster (Single-Zone) instance in the requested cloud provider and region. The instance is created asynchronously; poll [Describe](/cloud/api-reference/enterprise-byoa/cluster-single-zone/describe) until `status` is `RUNNING`.
