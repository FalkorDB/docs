...
![The Import/Export RDB tab with the task list and the Export RDB, Import RDB, and Schedules buttons](/cloud/images/export-import/import-export-tab.png)
...
![Export RDB dialog with Temporary link selected](/cloud/images/export-import/export-temporary-link.png)

When the task completes, select **Download** in the **Output** column to retrieve the file.

![Task list showing a completed export with a Download link](/cloud/images/export-import/export-task-download.png)
...
![Export RDB dialog with Google Cloud Storage selected](/cloud/images/export-import/export-google-cloud-storage.png)
...
![Export RDB dialog with Amazon S3 selected](/cloud/images/export-import/export-amazon-s3.png)
...
![Import RDB dialog with Upload file selected and the erase caution](/cloud/images/export-import/import-upload-file.png)
...
![Import RDB dialog with Google Cloud Storage selected](/cloud/images/export-import/import-google-cloud-storage.png)
...
![Import RDB dialog with Amazon S3 selected](/cloud/images/export-import/import-amazon-s3.png)
...
<AccordionGroup>
  <Accordion title="How is an export different from a snapshot or an automatic backup?">
    An export is an **RDB file you create or schedule**, sent to a download link or your own cloud storage. Automatic backups and snapshots are taken by the platform and are managed separately. See your tier page for those policies.
  </Accordion>
  <Accordion title="Does an import overwrite my current data?">
    Yes. The instance is **erased before the import**. FalkorDB takes a safety copy of the current data before clearing it, then verifies the imported key count after the load.
  </Accordion>
  <Accordion title="Which destinations and sources are supported?">
    Export goes to a **temporary download link**, **Google Cloud Storage**, or **Amazon S3**. Import reads from an **uploaded file**, a **URL**, **Google Cloud Storage**, **Amazon S3**, or **another instance**.
  </Accordion>
  <Accordion title="Can I schedule exports on any tier?">
    Scheduled exports are available on the **Pro** and **Enterprise** tiers. On lower tiers the Schedules button is disabled.
  </Accordion>
  <Accordion title="Are my cloud storage credentials stored?">
    No. Credentials you enter for your own bucket are used only to reach that bucket for the task. They are not kept on the task record.
  </Accordion>
</AccordionGroup>
